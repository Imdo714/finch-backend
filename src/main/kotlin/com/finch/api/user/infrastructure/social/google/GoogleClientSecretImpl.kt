...
import com.finch.global.exception.handleException.GoogleInvalidUserResponseException
...
class GoogleClientSecretImpl(
    ...
    override fun getGoogleUserInfo(accessToken: String): GoogleUserInfoDto {
        return restClient.get()
            .uri(GOOGLE_USER_INFO_URL)
            .header("Authorization", "Bearer $accessToken")
            .retrieve()
            .onStatus({ it.is4xxClientError }) { _, response ->
                log.error { "[GoogleAuth] 유저 정보 요청 실패 (4xx) - 상태코드: ${response.statusCode}" }
                throw InvalidAuthorizationException()
            }
            .onStatus({ it.is5xxServerError }) { _, response ->
                log.error { "[GoogleAuth] 유저 정보 요청 실패 (5xx) - 구글 서버 오류" }
                throw GoogleInvalidUserResponseException()
            }
            .body(GoogleUserInfoDto::class.java)
            ?: throw GoogleInvalidUserResponseException()
    }
